                ...
                >
                    <View>
                        <View className="flex flex-row justify-between">
                            <View className="bg-[#322d42] w-[50%] py-3 items-center border-b border-slate-800">
                                <Text className="text-gray-50">Pembayaran</Text>
                            </View>
                            <View onTouchStart={() => navigate.navigate("TopUp")} className="bg-[#d2fab2] w-[50%] py-3 items-center">
                                <Text>Top Up</Text>
                            </View>
                        </View>
                        {Object.keys(onePembayaran)?.length !== 0 && onePembayaran.status ?
                            <CardSpp
                                ...
                                total={onePembayaran.loan.jumlah}
                        ...
                        <Pressable onTouchStart={() => setShowAcceptment(true)} className={`mx-auto bg-[#e4b417] px-[43vw] py-3 rounded-lg mb-1 ${onePembayaran.status ? "block" : "hidden"}`}>
                            <Text>Bayar</Text>
                        </Pressable>
                    </View>
                    <View className="mx-auto w-full bg-slate-800 flex flex-row py-2 px-3 mt-1">
                        <Image source={titleHistory} className="w-9 h-9 my-auto"/>
                        <Text className=" text-slate-50 mx-1 p-2 text-lg">Catatan Pembayaran</Text>
                    </View>
                    <ScrollView className="mx-3 mb-20">
                        {[...history].sort((a, b) => {
